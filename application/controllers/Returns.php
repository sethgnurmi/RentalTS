...
class Returns extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('Returns/orders_overview', $this->data);
	}
	...
	public function Order()
	{
		if($this->uri->segment(3) && is_numeric($this->uri->segment(3)) && $this->order_model->getActor($this->uri->segment(3)))
		{
			...
			if($this->input->post() && isset($this->input->post()['ConfirmOrderReturn']))
			{
				$lineItemList = $this->order_model->getLineItemsList($actor['actor_id']);
				foreach($lineItemList as $line_item)
				{
					$stockItemData = array('stock_item_id' => $line_item['stock_item'], 'status' => 1);
					$this->shipping_model->updateStockItem($stockItemData);
				}

				$actorData = array('actor_id' => $this->uri->segment(3), 'order_status' => 2);
				$this->shipping_model->updateActor($actorData);
				
				redirect('/Returns', 'refresh');
			}
		...
		$this->load->view('Returns/order_form', $this->data);
	}
}
